...
fn main() {
    ...
    let mut my_barcode_str_clone = args.scrna_barcode.clone().unwrap();
    // let my_barcode_candidate = get_barcode_str(&my_str,&args.scrna_barcode.unwrap(),1);
    let mut my_whitelist = get_barcode_whitelist(&args.barcode_file);
    ...
    let mut my_phase_values: Vec<i32> = vec![0,1,2,3];
    let mut my_final_check: HashMap<String, Vec<String>>  = HashMap::new();
    // std::process::exit(0);
    //////////////////////
    let mut n: u32 = 0;
    let mut slen: u64 = 0;
    ...
    while let Some(record) = reader.next() {
        ...
        let my_final_check = phased_check(String::from_utf8_lossy(&seqrec.seq()).to_string(), my_barcode_str_clone.clone(),my_whitelist.clone(),my_phase_values.clone());
        ...
        println!("{},{},{},{},{},{},{},{},{},{},{},{}", String::from_utf8_lossy(&seqrec.id()).to_string(),barcode_seq[0],best_barcode[0],best_dists[0],best_tiered_seqs[0],best_dists[1],best_tiered_seqs[1],best_dists[2],best_tiered_seqs[2],best_dists[3],best_tiered_seqs[3],total_dist);
